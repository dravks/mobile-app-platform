...
import { FormNotice, SuggestionForm } from "@/components/public/forms";
...
export default async function ProjectSuggestionsPage({
  ...
  if (!project || !project.suggestionsEnabled) notFound();
  return (
    <main className="container grid gap-8 py-12 lg:grid-cols-[0.8fr_1.2fr]">
      <div>
        ...
        <h1 className="text-4xl font-black">{t(locale).common.suggestions}</h1>
        <p className="mt-5 leading-8 text-slate-600">{t(locale).project.contactCta}</p>
      </div>
      <div className="grid gap-4">
        <FormNotice locale={locale} success={state.success} error={state.error} />
        <SuggestionForm locale={locale} projectId={project.id} redirectTo={`/${locale}/projects/${project.slug}/suggestions`} />
      </div>
    </main>
  );
}
